    ...
    <table style="white-space: nowrap;width: 90%">
      ...
      <tr style="height: 35px; border-top: 1px solid">
        ...
        <td>
          ...
          >
            ...
            <template #default>
              <div v-for="(subCategory, subIndex) in category.children" :key="subIndex" style="margin-bottom: 1px">
                <el-button
                    link
      ...
      <tr style="height: 35px;border-top: 1px solid">
       ...
       <span style="margin-left: 22px">
            券后价
            <el-input size="small" v-model="filt.qhj_f" type="number"
                      :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
            />
            ~
            <el-input size="small" v-model="filt.qhj_t" type="number"
                      :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
            />
          </span>
            <span style="margin-left: 10px">
           佣金比例
            <el-input placeholder="%" size="small" v-model="filt.cr_f" type="number"
                      :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
            />
            ~
            <el-input placeholder="%" size="small" v-model="filt.cr_t" type="number"
                      :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
            />
          </span>
            <span style="margin-left: 10px">
          佣金  ≥
          <el-input size="small" v-model="filt.cv" type="number"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          </span>
            <span style="margin-left: 10px">
          销量  ≥
          <el-input size="small" v-model="filt.sale_num" type="number"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          </span>
            <span style="margin-left: 10px">
          券面额
          <el-input size="small" v-model="filt.coupon_v_f" type="number"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          ~
          <el-input size="small" v-model="filt.coupon_v_t" type="number"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          </span>
            ...
            <span style="margin-left: 10px">
          券到期时间  ≥
          <el-input size="small" v-model="filt.qgt" type="number"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          </span>
            ...
            <span style="margin-left: 10px">
            体验分  ≥
          <el-input size="small" v-model="filt.experience_score"
                    :class="config.side_mode ==='normal'? 'input_tight':'input_loose'"
          />
          </span>
        ...
        <td>
          <div style="display: flex; align-items: center; width: 100%">
            ...
            >
              ...
              <el-button icon="Delete" link style="margin-left: 20px" type="primary" size="small"
                         @click="condition.data_from=null"
              >
                清除
              </el-button>
            </el-popover>
            ...
            >
              <template #reference>
                <span class="popover-reference" :style="{color: shopShow ? 'var(--el-color-primary)' : null }"
                > {{ shopName }}&nbsp;<el-icon><ArrowDown/></el-icon></span>
              </template>
            ...
            >
              <template #reference>
                <span class="popover-reference" :style="{color: biaoxianShow ? 'var(--el-color-primary)' : null }"
                > {{ biaoxianName }}&nbsp;<el-icon><ArrowDown/></el-icon></span>
              </template>
              <el-radio-group v-model="condition.bx_bdan" class="ml-4">
                <el-radio :value="1">实时榜单</el-radio>
                <el-radio :value="2">30天榜单</el-radio>
              </el-radio-group>
              <el-button icon="Delete" link style="margin-left: 20px" type="primary" size="small"
                         @click="condition.bx_bdan=null"
              >清除
              </el-button>
...
const submitKG = () => {
  if (typeof keyword.value === "string" && keyword.value.trim() !== "") {
    condition.kw = keyword.value.trim();
  }
};
// 监听输入框是否发生变化
watch(keyword, (newV) => {
...
const onFilt = () => {
  ...
  condition.couv_f = Number(filt.coupon_v_f) === 0 ? null : Number(filt.coupon_v_f)
...
const methods = {
  ...
  sortZH: () => {
    if (condition.order_by !== "zh") {
      condition.order_by = "zh"
    }
  },
  sortDaySales: () => {

      ...
      <div class="mb-8">
        ...
        <div class="mt-2 text-sm p-2 flex items-center gap-2">
          <div
              ...
              :style="`background:${item}; border: 1px solid black;`"
...
<!--        请注意，所有配置请保存到本地文件的-->
<!--        <el-tag>config.json</el-tag>-->
<!--        文件中，否则刷新页面后会丢失配置-->
<!--      </el-alert>-->
      <div>
<!--        <el-button type="primary" class="mt-4" @click="copyConfig">复制配置</el-button>-->
        <el-button type="primary" class="mt-4" @click="saveToMysql">保存到数据库</el-button>
      </div>
...
import {setUserSettingData} from "@/api/user";
...
const saveToMysql = () => {
  setUserSettingData(config.value).then((res) => {
    if (res.code === 0) {

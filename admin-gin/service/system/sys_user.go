...
func (userService *UserService) Register(u system.SysUser) (userInter system.SysUser, err error) {
	...
	u.UUID = uuid.Must(uuid.NewV4())     // uuid.Must() 失败直接panic
	err = global.GVA_DB.Create(&u).Error // 关联新增 user_authority
	return u, err
}
...
func (userService *UserService) SetUserAuthorities(id uint, authorityIds []uint) (err error) {
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		// 函数在中使用事务： 如果返回err, 则回滚。 返回nil 提交事务。
		var user system.SysUser
...
// 获取用户前端settingData
func (userService *UserService) GetUserSettingData(ID uint) (data system.SysSetting, err error) {
	err = global.GVA_DB.Where("user_id = ?", ID).First(&data).Error
	return
}

// 设置用户前端settingData
func (userService *UserService) SetUserSettingData(req system.SysSetting) (err error) {
	// 尝试使用 Save 方法，该方法可以在记录存在时更新，不存在时创建
	var id uint
	err = global.GVA_DB.Model(req).Where("user_id = ?", req.UserID).Select("id").Scan(&id).Error
	if err != nil {
		global.GVA_LOG.Error(err.Error())
		return err
	}
	req.ID = id
	err = global.GVA_DB.Save(&req).Error
	if err != nil {
		return err
	}
	return nil
